package com.ngocanh.spring5recipeapp.bootstrap;

import com.ngocanh.spring5recipeapp.domain.*;
...
public class RecipeBootstrap implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<Recipe> getRecipes(){
        ...
        if (!teaSpoonUomOptional.isPresent()){
            throw new RuntimeException("Exptected UOM not found");
        }
        ...
        Optional<Category> mexicanCategoryOptianl = categoryRepository.findByDescription("Mexican");

        if (!mexicanCategoryOptianl.isPresent()){
            throw new RuntimeException("Expected Category not found");
        }

        Category americanCategory = americanCategoryOptional.get();
        Category mexicanCategory  = mexicanCategoryOptianl.get();
